...
void Get_LotNum()
{
	for (int SiteCntr = 1; SiteCntr <= MAX_SITES_NUM; SiteCntr += SITE_PER_CONTROLLER_NUM)
	{
		...
		if (VERBOSE) output("START_OF_LOT_EVENT: set LOT_NUM=%s WFR_SLOT_ID=%s to site%d", LOT_NUM, WFR_SLOT_ID, SiteCntr);
	}
}
BOOL_VARIABLE(FK1, FALSE, "FK1")
{
	if (OnHost())
	{
...
INT64_VARIABLE(iv_init_dut_mask, 0xff, "Actual Duts being tested"){};
...
CSTRING_VARIABLE(DLOG_FILE_PATH, "D:\\Datalogs", "File path for datalog files") {};
...
void after_test_done_process(void);
...
void after_test_done_process(void)
{
	...
	for (int SiteCntr = 1; SiteCntr <= MAX_SITES_NUM; SiteCntr += SITE_PER_CONTROLLER_NUM)
	{
		...
		for (int cntr = 0; cntr < MAX_DUT_NUM_PER_SITE; cntr++)
		{
			...
			{
				//sscanf(ResultsFromDut, "X=%d,Y=%d,S=%d,D=%d,R=%d,T=%d,N=%s", &x_,&y_,&s_,&b_,&r_,&t_,binname); // this is causing Heap Corruption
			...
			output("ResultsFromDut = %s", ResultsFromDut);
		...
		if (VERBOSE) output("cDutDatas=%s", cDutDatas);
	}
}
...
void process_test_results(TestBin* current_test_bin, int sbin, int hbin, PFState pf)
{
	...
	// Loop through all the active duts, any dut that is in the active duts
	// will be assigned the current bin.
...
AFTER_TESTING_BLOCK(tp_ATB)
{
	...
	for (int cntr = 0; cntr <= max_dut(); cntr++)
	{
		...
		StrBfr.Format("X=%d,Y=%d,S=%d,D=%d,R=%d,T=%d,N=%s ", x, y, dut_bin[cntr].sbin, dut_bin[cntr].hbin, dut_bin[cntr].result, itime, dut_bin[cntr].name);
		RESULTS_STR += StrBfr;
	}

	if (VERBOSE) output("RESULTS_STR= %s", RESULTS_STR);
...
CSTRING_VARIABLE(Magnum_OI, "", "Magnum_OI")
{
