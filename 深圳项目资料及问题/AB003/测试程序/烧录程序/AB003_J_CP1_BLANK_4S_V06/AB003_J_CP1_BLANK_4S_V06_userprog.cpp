#include "stdafx.h"
#include "AB003_J_CP1_BLANK_4S_V06_userprog.h"
/*=========================================
 |   Devolop : zhangwei                   |
 |   Version : AB003_J_CP1_BLANK_4S_V06   |
 |   Date    :                            |
 ==========================================*/
...
bool bPass[MAX_TESTING_SITE];
	...
	long PROGRAM_ST,PROGRAM_SP;
	long VERIFY_ST,VERIFY_SP;
	long VERIFY2_ST,VERIFY2_SP;
...
//	BLANKCHK_SP=peb.LoadPattern(BLANKCHK_ST,"BLANKCHK_02.PTN");
	...
	PROGRAM_ST=LOFC_SP+1;
	PROGRAM_SP=peb.LoadPattern(PROGRAM_ST,"55aa_isp_01.PTN"); 

	VERIFY_ST=PROGRAM_SP+1;
	VERIFY_SP=peb.LoadPattern(VERIFY_ST,"55aa_isr_01.PTN");

	VERIFY2_ST=VERIFY_SP+1;
	VERIFY2_SP=peb.LoadPattern(VERIFY2_ST,"55aa_isr_nor_01.PTN");
...
void DISP12(void)
{
	...
	//<--- Test P16
	peb.ppmu.FI("P16",-0.000001,CPmu::I_RNG_40UA,4,0);
	util.dlyms(1);
	peb.ppmu.MV("P16",1);
...
void PROGRAM_55AA_ISP(void)
{
    ...
    //<--- Run Pattern	
	peb.SetTestPatternOption(PROGRAM_ST,PROGRAM_ST,1,false);
	peb.TestPattern(PROGRAM_ST,PROGRAM_SP,bPass);
	util.FuncLog(true,bPass);
	}

void VERI_55AA_ISR(void)
{
	//<--- Setting Level
	peb.SetPinLevel("PATTERN_PIN","3.3V");
	peb.SetPinLevel("X32KI","1.8V");
    ...
    //<--- Run Pattern	
	peb.SetTestPatternOption(VERIFY_ST,VERIFY_ST,1,false);
	peb.TestPattern(VERIFY_ST,VERIFY_SP,bPass);
	util.FuncLog(true,bPass);
	}

void VERI_55AA_ISR_NOR(void)
{
    //<--- Setting Level
	peb.SetPinLevel("PATTERN_PIN","3.3V");
	peb.SetPinLevel("X32KI","1.8V");
    ...
    //<--- Run Pattern	
	peb.SetTestPatternOption(VERIFY2_ST,VERIFY2_ST,1,false);
	peb.TestPattern(VERIFY2_ST,VERIFY2_SP,bPass);
	util.FuncLog(true,bPass);
